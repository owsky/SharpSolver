 ...
 * Prelude.fs: definizioni iniziali
...
let get_assembly_attribute<'T when 'T :> Attribute> (asm: System.Reflection.Assembly) =
    let t = typeof<'T>

    try
        let rex =
            Text.RegularExpressions.Regex("(System.Reflection.Assembly)(\w+)(Attribute)")

        let name = rex.Match(t.FullName).Groups.[2].Value
        let attrs = asm.GetCustomAttributes(t, false)
        let att = attrs[0] :?> Attribute in
...
type rational(p: int, ?q: int) =
    ...
    static member Parse(s: string) =
        ...
        | _ -> failwithf $"rational.Parse: invalid syntax: {s}"
    ...
    static member (+)(m: rational, n: rational) =
        rational (m.N * n.D + n.N * m.D, m.D * n.D)
